...
#set -e causes the shell script to exit immediately if any command returns a non-zero exit status (i.e., fails). This helps prevent the script from continuing after errors.
set -e
...
if [ ! -c /dev/net/tun ]; then
  echo "/dev/net/tun is missing. TUN/TAP support is required."
  exit 1
fi
